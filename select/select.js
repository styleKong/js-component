...
; (function (object) {
    ...
    createInput(select) {
        ...
        <input name="${name}" data-id="${checked ?.value ?? ''}" value="${checked ?.innerHTML ?? ''}" placeholder="${placeholder}" ${change ? ' onchange="' + change + '"':''} readonly />
    ...
    selectClick(event) {
        ...
        if (event.target.classList.contains('k-select-drop-item')) {
            ...
            input.setAttribute('data-id', event.target.getAttribute('data-value'))
            if(input.hasAttribute('onchange'))input.onchange(input)
        }
    },
    /**
     * 点击元素选中方法
